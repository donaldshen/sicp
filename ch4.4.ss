...
(rule (end-with-grandson (?x . ?rest))
      (end-with-grandson ?rest))
(rule ((grandson) ?x ?y)
      (grandson ?x ?y))
(rule ((great . ?rel) ?x ?y)
...
(define (rename-variables-in rule)
  (let ((rule-application-id (new-rule-application-ed)))
...
(define (get-indexed-assertions pattern)
  (get-stream (index-key-of-pattern pattern)
	      'assertion-stream))
...
(define (fetch-rules pattern frame)
  (if (use-indexed? pattern)
      (get-indexed-rules pattern)
      (get-all-rules)))
(define (get-all-rules)
  THE-RULES)
(define (get-indexed-rules assertion)
